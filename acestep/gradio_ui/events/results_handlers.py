...
def generate_with_progress(
    ...
    for idx in range(8):
        ...
        if path:
            # Pass path directly - Gradio's Audio.postprocess handles conversion
            audio_playback_updates.append(gr.Audio(value=path, label=f"Sample {idx+1} (Ready)"))
...
def navigate_to_previous_batch(current_batch_index, batch_queue):
    ...
    for idx in range(8):
        if idx < len(real_audio_paths):
            ...
            # Pass path directly - Gradio's Audio.postprocess handles conversion
            audio_updates.append(gr.Audio(value=audio_path))
        else:
